...
class HMC(nn.Module):
    ...
    def __init__(self, dim: int, logp: Callable, T: int=10,  L: int=5, chains: int=1, chains_sksd: int=30,
                mu0: torch.Tensor=None, var0: torch.Tensor=None, vector_scale=True):
        """
        HMC initialization
        ...
        Args:
            ...
            mu0 (torch.Tensor, optional): mean of the initial proposal. Defaults to None.
            var0 (torch.Tensor, optional): var of the initial proposal. Defaults to None.
            vector_scale (bool, optional): learning a vector inflation (True) or a scalar (False). Defaults to None.
        """
        super().__init__()
    ...
    def init_random_params(self, T: int, dim: int, mu0: torch.Tensor=None, var0: torch.Tensor=None):
        ...
        # Proposal can also be tuned but we see empirically that this is not stable
        self.mu0 = mu0
    ...
    def evaluate_sksd(self, mu0: torch.Tensor, var0: torch.Tensor) -> torch.Tensor:
        ...
        sksd = sksd[torch.isfinite(sksd)].mean()
    ...
    def loss(self, mu0, var0):

        z, _ = self.sample(mu0, var0, self.chains)
        objective = self.logp(z)
        sksd = self.evaluate_sksd(mu0, var0)
        loss = -objective.clone()# + sksd

        return loss, objective, sksd

    def fit(self, steps=1000, *args, **kwargs):
        """
        ...
        for e in t:
            ...
            t.set_description('HMC (objective=%g)' % -_loss.mean().detach().numpy())
